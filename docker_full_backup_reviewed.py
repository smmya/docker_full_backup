...
def archive_directory_contents(
    source: pathlib.Path,
    output: pathlib.Path,
    threads: str,
    ...
    xz_compress_pipeline(cmd, output, compression_level, threads)


def archive_single_path(
    source: pathlib.Path,
    output: pathlib.Path,
    threads: str,
    compression_level: str,
...
def backup_command(args: argparse.Namespace) -> None:
    ...
    compression_level = MAX_DATA_XZ_LEVEL if args.max_compress else DEFAULT_FINAL_XZ_LEVEL
    if args.max_compress:
        info("XZ compression preset: -9e (--max-compress)")
    else:
        info("XZ compression preset: -6 (default)")
            ...
            for mount in current.get("Mounts") or []:
                ...
                if mtype == "bind":
                    ...
                    if not os.path.lexists(str(src_path)):
                        ...
                        if kind == "directory":
                            archive_directory_contents(src_path, archive_abs, args.xz_threads, compression_level)
                            archive_layout = "contents"
                        else:
                            archive_single_path(src_path, archive_abs, args.xz_threads, compression_level)
                    ...
                    archive_directory_contents(pathlib.Path(mountpoint), archive_abs, args.xz_threads, compression_level)
                    record = {
                        "id": mount_id,
                        "type": "volume",
                        "source": source,
            ...
            if not args.no_logs:
                ...
                if log_path_raw:
                    ...
                    if log_files:
                        ...
                        xz_compress_pipeline(cmd, work / rel, compression_level, args.xz_threads)
                        log_record = {
                            "id": log_id,
                            "container": name,
...
""",
    ...
    p_backup.add_argument(
        "--max-compress",
        action="store_true",
        help="use xz -9e for all archive layers instead of the default xz -6",
    )
